...
export default class Table extends Controller {
    ...
    get $table() {
        return this.$element.find('.table__table');
    }

    get $rows() {
        if (!this._$rows) {
            this._$rows = this.$element.find('.table__row');
        }
        return this._$rows;
    }
    ...
    renderRowPlaceholders() {
        let rowsToBeRendered = this.model.count - this.$element.find('.table__row').length;
        let html = this.$element.find('.table__row-template').html();
